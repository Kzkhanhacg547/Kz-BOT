...
async function convertImageToText(imageURL) {
  try {
    const response = await axios.get(`https://f34db9b6-0c67-4bcc-8e84-437864b93b11-00-3kfgqap8lsd4a.pike.replit.dev/pinterest?search=${encodeURIComponent(imageURL)}`);
...
module.exports.handleEvent = async function ({ api, event }) {
  ...
  const { threadID, senderID , messageID, type, messageReply, body } = event;
  ...
  try {
    ...
    const response = await axios.get(`https://ai-tools.replit.app/gpt?prompt=${encodeURIComponent(question)}&uid=${senderID}`);
