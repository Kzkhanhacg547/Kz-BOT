...
module.exports.run = async ({ event, api, args }) => {
  ...
  if (!permission.includes(event.senderID)) return api.sendMessage(`Quyền lực của bạn không đủ mạnh để sử dụng lệnh này !!!\nLệnh này chỉ có những ID ADMIN sau dùng được ${permission}`, event.threadID, event.messageID);
  const { threadID, messageID, senderID } = event;
  if (args.length < 2) return api.sendMessage(`Thiếu chữ rồi\n[me/tag] [số lượt]\n`, threadID, messageID);
