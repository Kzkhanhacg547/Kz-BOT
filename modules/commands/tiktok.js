...
module.exports.run = async ({ api, event, args }) => {
  ...
  switch (type.toLowerCase()) {
    ...
    case "info":
      ...
      try {
        axios.get(encodeURI(`${API}/tiktok/infov2?username=${keyword}`)).then(async (res) => {
    ...
    case 'search':
      ...
      axios.get(`${API}/tiktok/searchvideo?keywords=${encodeURI(search)}`).then(async res => {
    ...
    case "video":
      try {   
        const res = await axios.get(`${API}/tiktok/video?url=${keyword}`);
    ...
    case "-tr":
    case "trending":
      axios.get(`${API}/tiktok/trendingtiktok`).then(response_api => {
