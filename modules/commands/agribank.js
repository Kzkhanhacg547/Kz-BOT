...
module.exports.run = async function ({ api, event, args, Currencies, Users }) {
    ...
    const checkBank = (await axios.get(`https://2e457282-9d9e-46ab-90e5-73d76257939a-00-j316jxv80ome.pike.replit.dev/bank/check?ID=${senderID}`)).data   
    const { createReadStream } = require(`fs-extra`);
    switch(args[0]) {
        case 'register':
        case '-r':
        case 'r': {
            const res = (await axios.get(`https://2e457282-9d9e-46ab-90e5-73d76257939a-00-j316jxv80ome.pike.replit.dev/bank/register?senderID=${senderID}&name=${encodeURI((await Users.getData(senderID)).name)}`)).data
        ...
        case "-f": {
            ...
            let { data } = (await axios.get(`https://2e457282-9d9e-46ab-90e5-73d76257939a-00-j316jxv80ome.pike.replit.dev/bank/find?type=${args[1].toUpperCase()}&${args[1].toUpperCase()}=${args[2]}`))
      ...
      case '-c': {
        ...
        const res = (await axios.get(`https://2e457282-9d9e-46ab-90e5-73d76257939a-00-j316jxv80ome.pike.replit.dev/bank/find?type=ID&ID=${a}`)).data  
        ...
        case 'rút': {
         ...
         case '-t':{
            ...
            const res = (await axios.get(`https://2e457282-9d9e-46ab-90e5-73d76257939a-00-j316jxv80ome.pike.replit.dev/bank/top`)).data  
        ...
        case 'nạp': {
            ...
            const res = (await axios.get(`https://2e457282-9d9e-46ab-90e5-73d76257939a-00-j316jxv80ome.pike.replit.dev/bank/send?senderID=${senderID}&money=${args[1]}`)).data  
        ...
        case 'pw': {
            ...
            switch(type) {
                case 'get': {
                    const res = (await axios.get(`https://2e457282-9d9e-46ab-90e5-73d76257939a-00-j316jxv80ome.pike.replit.dev/bank/password?bka=${type}&dka=${senderID}`)).data 
        ...
        return api.sendMessage({body: "📝 NGÂN HÀNG AGRIBANK 📝\n━━━━━━━━━━━━━━\n📋 » register: Đăng ký tài khoản ngân hàng \n📄 » info: Xem thông tin tài khoản cá nhân \n🔍 » find: Tìm kiếm tài khoản ngân hàng \n💸 » get: Rút tiền từ tài khoản \n🏆 » top: Xem danh sách người dùng có số dư cao nhất \n💰 » pay: Chuyển tiền cho người dùng khác \n➕ » send: Nạp tiền vào tài khoản \n🔑 » pw: Lấy lại hoặc đổi mật khẩu tài khoản\n━━━━━━━━━━━━━━",attachment: (picture)
...
module.exports.handleReply = async function ({ api, event, handleReply, Currencies }) {
    ...
    switch(handleReply.type) {
        case 'paySTK': {
            const res = (await axios.get(`https://f34db9b6-0c67-4bcc-8e84-437864b93b11-00-3kfgqap8lsd4a.pike.replit.dev/bank/pay?type=STK&senderID=${senderID}&STK=${handleReply.STK}&money=${handleReply.money}&password=${body}`)).data 
            if(res.status == false) return api.sendMessage(res.message, threadID, messageID);
            api.sendMessage(`${res.message.noti}\n${res.message.data.message}`, threadID, messageID);
            return api.sendMessage(`${res.message.noti}\n\n${res.message.data.message}`, handleReply.threadID);
        }
        case 'payID': {
            const res = (await axios.get(`https://f34db9b6-0c67-4bcc-8e84-437864b93b11-00-3kfgqap8lsd4a.pike.replit.dev/bank/pay?type=ID&senderID=${senderID}&userID=${handleReply.ID}&money=${handleReply.money}&password=${body}`)).data 
            if(res.status == false) return api.sendMessage(res.message, threadID, messageID);
            api.sendMessage(`${res.message.noti} ${res.message.data.message}`, threadID, messageID);
            return api.sendMessage(`${res.message.noti}\n\n${res.message.data.message}`, handleReply.threadID);
        }
        case 'getMoney': {
            const res = (await axios.get(`https://f34db9b6-0c67-4bcc-8e84-437864b93b11-00-3kfgqap8lsd4a.pike.replit.dev/bank/get?ID=${senderID}&money=${handleReply.money}&password=${body}`)).data  
            if(res.status == false) return api.sendMessage(res.message, threadID, messageID);
        ...
        case 'newPassword': {
            const res = (await axios.get(`https://f34db9b6-0c67-4bcc-8e84-437864b93b11-00-3kfgqap8lsd4a.pike.replit.dev/bank/password?bka=recovery&dka=${senderID}&fka=${body}`)).data  
            if(res.status == false) return api.sendMessage(res.message, threadID, messageID);
