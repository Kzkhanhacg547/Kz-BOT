...
moment.tz('Asia/Ho_Chi_Minh').format('dddd');
  ...
  const res = await axios.get(`${API}/cadao`);
var cadao = res.data.data;
  const namebot = global.config.BOTNAME;
  const kz = await axios.get(`${API}/thinh`);
var thinh = kz.data.data;
  const kz2 = await axios.get(`${API}/danhngon`);
var danhngon = kz2.data.data;
  ...
  const kzhoi = await axios.get(`${API}/hoi`);
  var cauhoi = kzhoi.data.data;
  const ex = await axios.get(`${API}/kinhnghiem`);
...
━━━━━━━━━━━━━━━━━━\n\n𝟭. 𝐊𝐳_𝐊𝐡á𝐧𝐡 ( 𝗮𝗱𝗺𝗶𝗻 𝗰𝗵𝗶́𝗻𝗵 )\n𝟮. 𝐘𝐞̂́𝐧 𝐍𝐡𝐢👾\n\n𝗥𝗲𝗽𝗹𝘆 𝘁𝗶𝗻 𝗻𝗵𝗮̆́𝗻 𝗻𝗮̀𝘆 𝘁𝗵𝗲𝗼 𝘀𝗼̂́ đ𝗲̂̉ 𝘅𝗲𝗺 𝘁𝗵𝗼̂𝗻𝗴 𝘁𝗶𝗻 𝗰𝘂̉𝗮 𝗮𝗱𝗺𝗶𝗻 𝗯𝗮̣𝗻 𝗺𝘂𝗼̂́𝗻 𝘅𝗲𝗺`
        return api.sendMessage({body: msg, attachment: (await axios.get((await axios.get(`${API}/gaiaodai`)).data.url,  {
                    responseType: 'stream'
                })).data},event.threadID, 
                              (error, info) => {
...
url: (await global.nodemodule["axios"](`${API}/ad`)).data.url,
method: "GET",
responseType: "stream"
})).data
...
url: (await global.nodemodule["axios"](`${API}/vdnhac`)).data.url,
method: "GET",
responseType: "stream"
})).data
},event.threadID, event.messageID)
