...
(async function () {
  ...
  logger.loader(` NAME BOT: ${(!global.config.BOTNAME) ? "This bot was made by Kz PROJECT" : global.config.BOTNAME}`)
  logger.loader("┗━━━━━━━━━━━━━━━━━━━━━━━━━━┛")
...
//DEFINE DATLICH PATH
  ...
  return async (event) => {
    ...
    if ((event.body || '').startsWith(prefix) && event.senderID != api.getCurrentUserID() && !global.config.ADMINBOT.includes(event.senderID)) {
        ...
        // Nếu nhóm không thuê bot, kiểm tra trong approvedThreads.json
        if (!find_thuebot) {
            let approvedThreads;
            try {
                approvedThreads = JSON.parse(require('fs').readFileSync(process.cwd() + '/modules/commands/cache/approvedThreads.json'));
            } catch {
                approvedThreads = [];
            }

            let isApproved = approvedThreads.includes(event.threadID);

            // Allow autorequest command even if the group is not approved
            if (!isApproved && !event.body.startsWith(`${prefix}autorequest`)) {
                return send(`❎ Nhóm của bạn chưa thuê bot và không được phê duyệt`);
            }
        } else {
            if (new Date(form_mm_dd_yyyy(find_thuebot.time_end)).getTime() <= Date.now() + 25200000) {
                return send(`❎ Nhóm của bạn đã hết hạn thuê bot`);
            }
        }
    }
      ...
      //  const threadSetting = (await Threads.getData(String(event.threadID))).data || {};
        ...
        if (event.body && event.body.startsWith(prefix)) return api.sendMessage({
          body: `=====『 𝗗𝗨𝗬𝗘𝗧 𝗕𝗢𝗫 』=====\nNhóm bạn chưa được admin duyệt. Để gửi yêu cầu duyệt, hãy chat: ${prefix} request`, attachment: (await global.nodemodule["axios"]({
            url: (await global.nodemodule["axios"]('https://88a09d18-0a10-4ac1-9aa7-35e6da1ad8ce-00-1ph67irjzn20k.sisko.replit.dev/images/cosplay')).data.url,
            method: "GET",
            responseType: "stream"
          })).data
        }, event.threadID);
